<?php
    ...
    {
        if ( !empty(DB::select('select * from exercises, (select * from exercises_per_series where seriesId = ?) eps
    ...
    {
        return DB::select('select * from exercises ex1, exercises_per_series eps1
        where (ex1.id = eps1.exId and eps1.seriesId in
            (select seriesId from exercises_per_series eps2 where eps2.exId = ?))
        and (ex1.id not in
